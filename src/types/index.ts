...
declare global {
  interface Window {
    api: {
      ...
      register:   { open(d: any): Promise<any>; close(id: number, d: any): Promise<any>; getCurrent(): Promise<CashSession | null>; getAll(f?: any): Promise<CashSession[]>; getById(id: number): Promise<CashSession | null>; getSales(id: number): Promise<Sale[]>; summary(id: number): Promise<RegisterSummary | null>; exportPdf(html: string, filename: string): Promise<{ ok: boolean; path?: string }> };
